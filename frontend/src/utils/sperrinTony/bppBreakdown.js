/**
 * Sperrin Tony – BPP Breakdown tab.
 * Renders BPP (Building Perimeter Protection / or project-specific) breakdown section.
 ...
 */
export function buildBppBreakdown(spreadsheet, ctx) {
  const pfx = ctx.pfx ?? PROPOSAL_SHEET_PFX
  let row = ctx.currentRow ?? 1

  spreadsheet.updateCell({ value: 'BPP Breakdown' }, `${pfx}B${row}`)
