    ...
    formulasToApply.forEach((formulaInfo) => {
      ...
      try {
        if (section === 'demolition') {
          ...
          // Excavation subsection: foundation-type items (pile cap, strip footing, pilaster, pits, SOG, etc.) — same calculation as Foundation
          if (itemType === 'excavation_item' && parsedData?.foundationType) {
            ...
            spreadsheet.cellFormat({ color: getColumnBColor(row, parsedData) }, `B${row}`)
            if (parsedData.foundationType === 'buttress_takeoff') {
          ...
          if (itemType === 'foundation_sum') {
            const { firstDataRow, lastDataRow, subsectionName, isDualDiameter, excludeISum, excludeJSum, excludeKSum, cySumOnly, lSumRange, matSlabCombinedSum, lastDataRowForJ } = formulaInfo

            // Mat slab: J (mat only) + L (mat+haunch) on same row
          ...
          if (itemType === 'superstructure_repair_scope') {
            ...
            spreadsheet.cellFormat({ color: '#FF0000', fontWeight: 'bold', fontStyle: 'normal' }, `I${row}`)
            spreadsheet.cellFormat({ color: '#FF0000', fontWeight: 'bold', fontStyle: 'normal' }, `J${row}`)
            spreadsheet.cellFormat({ color: '#FF0000', fontWeight: 'bold', fontStyle: 'normal' }, `M${row}`)
            return
          }
